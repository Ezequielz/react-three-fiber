...
export function Model(props: any) {
  const { nodes, materials }: any = useGLTF("/models/Artics.glb");
  ...
  const timeline = useRef<any>() 
    ...
    // meshes ref
  const generalGroupRef = useRef<any>()
  const coverLeftRef = useRef<any>()
  const coverRightRef = useRef<any>()
  const coverInnerRightRef = useRef<any>()
  const inside1Ref = useRef<any>()
  const inside2Ref = useRef<any>()
  ...
  const page_4_ref = useRef<any>();
  const page_5_ref = useRef<any>();
  const page_6_ref = useRef<any>();
  ...
  useLayoutEffect(() => {
    ...
    const ConstructionAnimations = [
      ...
      {
        objectToAnimate:
          coverInnerRightRef.current.position,
        properties: {
          x: 2.96,
          y: -0.59,
          z: 0,
